...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        } else if (code === 'PWD_RESET_001') {
          setFormError('유효하지 않은 재설정 링크입니다. 재설정을 다시 요청해주세요.');
        } else if (code === 'PWD_RESET_002') {
          setFormError('재설정 링크가 만료되었습니다. 재설정을 다시 요청해주세요.');
        } else if (code === 'PWD_RESET_003') {
          setFormError('이미 사용된 재설정 링크입니다. 재설정을 다시 요청해주세요.');
        } else {
  ...
  if (tokenMissing) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <main className="w-full max-w-sm bg-white rounded-lg shadow-md p-8 text-center" aria-label="비밀번호 재설정">
          <p className="text-sm text-red-600 mb-4">유효하지 않은 재설정 링크입니다.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <main className="w-full max-w-sm bg-white rounded-lg shadow-md p-8" aria-label="새 비밀번호 설정">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">사용할 새 비밀번호를 입력하세요.</p>
        </div>
        ...
        {succeeded ? (
          <div role="status" className="text-center">
            ...
            <p className="text-sm font-medium text-gray-700 mb-1">비밀번호가 변경되었습니다.</p>
